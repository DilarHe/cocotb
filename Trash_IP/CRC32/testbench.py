...
async def testbench(dut):   
    ...
    rg_puchbuf_addr = 128        # puchbuf 数据的起始字节地址（假设为128）
    ...
    ram_bytes[rg_puchbuf_addr : rg_puchbuf_addr + n] = np.arange(n, dtype=np.uint8)
    ...
    dut.resetn.value = 0
    dut.start.value = 0
    dut.rg_puchbuf_addr.value = rg_puchbuf_addr >> 2
    dut.rg_unlen.value = 4 
